...
public class ShelterManagementFrame extends JFrame {
    ...
    private JPanel buildForm() {
        // Outer: form on CENTER, button panel on EAST — same pattern as VictimManagementFrame
        ...
        chkMedical  = check("Medical Aid");
        chkFood     = check("Food");
        chkWater    = check("Water");
        chkFood.setSelected(true); chkWater.setSelected(true);

        addRow(form, g, 0, "Name *",      txtName,      "City *",      txtCity);
        addRow(form, g, 1, "Location",    txtLocation,  "Status",      cmbStatus);
        addRow(form, g, 2, "Capacity",    spnCapacity,  "Occupied",    spnOccupied);
        addRow(form, g, 3, "In Charge",   txtInCharge,  "Contact",     txtContact);

        // Amenities row
        g.gridx = 0; g.gridy = 4; g.weightx = 0; g.gridwidth = 1;
        form.add(lbl("Amenities"), g);
        g.gridx = 1; g.gridwidth = 3; g.weightx = 1;
        JPanel amen = new JPanel(new FlowLayout(FlowLayout.LEFT, 18, 0));
        amen.setOpaque(false);
        amen.add(chkMedical); amen.add(chkFood); amen.add(chkWater);
        form.add(amen, g);
        g.gridwidth = 1;

        // Notes row
        g.gridx = 0; g.gridy = 5; g.weightx = 0; form.add(lbl("Notes"), g);
        g.gridx = 1; g.gridwidth = 3; g.weightx = 1; form.add(txtNotes, g);
        g.gridwidth = 1;

        outer.add(form, BorderLayout.CENTER);

        // ── Buttons on right (same as VictimManagementFrame) ──
        ...
        JButton btnAdd = UITheme.primaryButton("➕  Add Shelter",    UITheme.ACCENT_GREEN);
    ...
    private JPanel buildTable() {
        ...
        int[] widths = {50, 140, 100, 80, 80, 80, 80, 120, 130, 75, 60, 60};
        for (int i = 0; i < widths.length; i++)
            table.getColumnModel().getColumn(i).setPreferredWidth(widths[i]);

        table.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent e) {
                if (e.getClickCount() == 2) fillFormFromTable();
            }
        });
    ...
    private void addShelter() {
        try {
            ...
            dao.addShelter(s); loadData(); clearForm();
    ...
    private void updateShelter() {
        ...
        if (row < 0) { JOptionPane.showMessageDialog(this, "Please select a shelter first.", "No Selection", JOptionPane.WARNING_MESSAGE); return; }
        try {
            ...
            Shelter s = new Shelter(id, txtName.getText().trim(), txtLocation.getText().trim(),
                    ...
                    (String)cmbStatus.getSelectedItem(), txtContact.getText().trim(), txtInCharge.getText().trim(),
                    chkMedical.isSelected(), chkFood.isSelected(), chkWater.isSelected(), txtNotes.getText().trim());
            dao.updateShelter(s); loadData(); clearForm();
            JOptionPane.showMessageDialog(this, "Shelter updated successfully.", "Updated", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception ex) { JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE); }
    }
    ...
    private void deleteShelter() {
        ...
        if (JOptionPane.showConfirmDialog(this, "Remove shelter \"" + name + "\" (ID: " + id + ")?\n\nThis cannot be undone.",
                "Confirm Removal", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) != JOptionPane.YES_OPTION) return;
        try { dao.deleteShelter(id); loadData(); clearForm(); }
        catch (Exception ex) { JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE); }
    }
                ...
                try {
                    ...
                    for (Shelter s : list) {
                        tableModel.addRow(new Object[]{
                                s.getShelterId(), s.getName(), s.getCity(),
                    ...
                    int cap  = list.stream().mapToInt(Shelter::getCapacity).sum();
                    int occ  = list.stream().mapToInt(Shelter::getOccupied).sum();
                    lblStats.setText("  Total: " + list.size() + " shelters  |  Open: " + open
                            + "  |  Capacity: " + cap + "  |  Occupied: " + occ + "  |  Available: " + (cap - occ));
    ...
    private void clearForm() {
        ...
        chkMedical.setSelected(false); chkFood.setSelected(true); chkWater.setSelected(true);
    ...
    private void setCombo(JComboBox<String> c, String v) {
    ...
    private JCheckBox check(String t) {
        ...
        c.setBackground(UITheme.BG_CARD); c.setForeground(UITheme.TEXT_PRIMARY); c.setFont(UITheme.FONT_NORMAL);
        return c;
    }
