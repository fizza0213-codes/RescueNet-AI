...
public class AIChatbot {
    ...
    public static String getGeminiResponse(String userMessage) {
        try {
            ...
            // Getting the response stream (either input stream for success, or error stream for failure)
            ...
            // Agar status code 200 nahi hai, toh debug karne ke liye error print karein aur fallback dein
            if (responseCode != 200) {
                System.out.println("API Error (" + responseCode + "): " + sb.toString());
                return getOfflineFallback(userMessage);
            }
            ...
            System.out.println("Exception in API Call: " + e.getMessage());
            return getOfflineFallback(userMessage);
        }
    }
    ...
    // ── Updated JSON Parser (Fixed to handle spaces dynamically) ──
    private static String parseGeminiResponse(String json) {
        try {
            // Find the "text" key
            int textKeyIndex = json.indexOf("\"text\"");
            if (textKeyIndex == -1) {
                System.out.println("Parsing Error: 'text' key not found in JSON.");
                return getOfflineFallback("");
            }

            // Find the colon and the start quote dynamically
            int colonIndex = json.indexOf(":", textKeyIndex);
            int startQuoteIndex = json.indexOf("\"", colonIndex);

            if (startQuoteIndex == -1) {
                System.out.println("Parsing Error: Value start quote not found.");
                return getOfflineFallback("");
            }

            int textStart = startQuoteIndex + 1;
